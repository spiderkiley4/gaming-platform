  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-background">
      <div className="bg-surface p-8 rounded-lg shadow-lg w-96">
        <h2 className="text-2xl font-bold mb-6 text-on-surface text-center">
        ...
        <button
          ...
          className="w-full mb-4 p-2 bg-success text-on-surface rounded hover:bg-success/80"
        >
          Test API Connection
        </button>
        ...
        <button
          ...
          className="w-full mb-4 p-2 bg-primary text-on-surface rounded hover:bg-primary-container"
        >
          Test User State
        </button>
        ...
        <button
          ...
          className="w-full mb-4 p-2 bg-accent text-on-surface rounded hover:bg-accent-container"
        >
          Test getCurrentUser API
        </button>

        {error && (
          <div className="bg-error text-on-surface p-3 rounded mb-4 text-sm">
            {error}
          </div>
        )}
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          {step === 1 && (
            <>
              <div>
                <label className="block text-on-surface-variant mb-1">Username</label>
                <input
                  type="text"
                  name="username"
                  value={formData.username}
                  onChange={handleInputChange}
                  className="w-full p-2 bg-surface-variant rounded text-on-surface"
                  required
                  minLength={3}
                />
              </div>
          ...
          {(step === 2 || isLogin) && (
            <div>
              <label className="block text-on-surface-variant mb-1">Password</label>
              <input
                type="password"
                name="password"
          ...
          {!isLogin && step === 1 ? (
            <button
              ...
              className="w-full bg-primary text-on-surface p-2 rounded hover:bg-primary-container disabled:opacity-50 disabled:cursor-not-allowed"
              disabled={isLoading}
            >
              Continue
            </button>
            ...
            >
              {isLoading ? (
                <span className="flex items-center justify-center">
                  <svg className="animate-spin -ml-1 mr-2 h-4 w-4 text-on-surface" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
        ...
        {!isLogin && step === 2 && (
          <button
            onClick={handleBack}
            className="mt-4 text-on-surface-variant hover:text-on-surface-variant text-sm w-full text-center"
          >
            ← Back
          </button>
        )}
